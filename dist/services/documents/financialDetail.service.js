...
export const listOfFinancialDetailsService = (financialId) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        if (!financialId) {
            return new ApiResponse(404, "financial id is not found", false, null);
        }
        const detail = yield FinancialDetail.findOne({ financialId });
        if (detail && detail._id) {
            return new ApiResponse(200, "data is succefully fetched", true, detail);
        }
        const createdFd = yield FinancialDetail.create({
            financialId
        });
